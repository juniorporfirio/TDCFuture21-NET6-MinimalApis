...
builder.Services.AddMediatR(typeof(Talk));
...
//Endpoints
app.MapGet("/talk", async (IMediator mediator) => await mediator.Send(new GetAllTalkRequest()));
app.MapGet("/talk/{Id}", async (IMediator mediator, Guid Id) => await mediator.Send(new GetTalkRequest(Id)));
app.MapPost("/talk/new", async (IMediator mediator, Talk talk) => await mediator.Send(new AddTalkRequest(talk.Id, talk.Title, talk.Speaker, talk.Trail)));
